...
const columns = [
  ...
  },
  {
    ...
    width: 210,
    editable: true,   
    headerClassName: 'SPLallowance',
    renderCell: (params) => {    
      const { id, SPLallowance } = params.row;
      return <TextField className='SPLallowanceinput' value={SPLallowance} />;
    }
  },   
...
const handleStateChange = React.useCallback((params) => {
 ...
 console.log("State data is",params,"condition checking",Object.keys(currentState).length)
  try
  {
  if(Object.keys(currentState).length > 0 )
    {
      sessionStorage.setItem('dataGridState',JSON.stringify(currentState));
      sessionStorage.setItem('selectedEmployee',JSON.stringify(params.rowSelection));
    }
...
const SentSite={
  ...
  let config = {
    headers: {
      'Authorization': 'Bearer ' + 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJkYXRhIjoiMSIsImlhdCI6MTcxNzkwMTIwNywiZXhwIjoxNzE3OTg3NjA3fQ.ZcVQEkGv6Gj5pObuwKXlpVp3Wf9D_RxYlI2jeNZbe-s'
    }
  }
  ...
  React.useEffect(()=>{
    ...
    axiosHttp
  .post("/GetSiteEmp",SentSite,config)
  .then((res) => {
